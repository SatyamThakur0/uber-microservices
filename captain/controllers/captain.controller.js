...
export default class CaptainAuth {
    ...
    login = async (req, res) => {
        try {
            ...
            if (!captain)
                return res
                    .status(404)
                    .json({ ok: false, msg: "captain not found." });
    ...
    waitForNewRide = async (req, res) => {
        try {
            ...
            pendingWaitForRideRequest.push(res);
        } catch (error) {
            res.json({ ok: false, msg: error.message });
        }
    };
...
subscribeToQueue("new-ride", (data) => {
    ...
    pendingWaitForRideRequest.forEach((res) => {
        res.status(200).json({ ok: true, newRide });
    });
    pendingWaitForRideRequest = [];
    console.log(newRide);
});
